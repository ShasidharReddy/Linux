...
### 6.9 Loop Over Script Arguments
```bash
for arg in "$@"; do
  echo "Arg: $arg"
done
...
### 6.10 Reading File Line by Line
```bash
...
### 6.11 Reading Command Output with Process Substitution
```bash
...
### 6.14 Loop Control with Multiple Levels
```bash
for i in 1 2 3; do
  for j in a b c; do
    [[ $j == b ]] && break 2
...
### 6.15 Looping Safely Over Filenames
Avoid this:
