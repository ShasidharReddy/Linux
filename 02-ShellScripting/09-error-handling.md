...
### 9.13 Check Command Status Directly
```bash
...
### 9.16 Ignore Certain Errors Deliberately
```bash
...
### 9.18 Safer Temporary Resource Cleanup
...
### 9.19 Logging Failed Commands
```bash
trap 'printf "Failed command: %s\n" "$BASH_COMMAND" >&2' ERR
```

### 9.20 Retrying Failures
```bash
retry() {
  local attempts=$1
  shift
